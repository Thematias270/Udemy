...
operador = ''
precios_comida = [1.32, 1.65, 2.31, 3.22, 1.22, 1.99]
precios_bebida = [0.25, 0.99, 1.21, 1.54, 1.08, 1.10]
...
def recibo():
    ...
    for postre in texto_postres:
        if postre.get() != '0':
            texto_recibo.insert(
                END, f'{lista_postres[x]}\t\t{postre.get()}\t${int(postre.get()) * precios_postres[x]}\n')
        x += 1
    ...
    texto_recibo.insert(
        END, f' Costo de los postres: \t\t\t{var_costo_postres.get()}\n')
    texto_recibo.insert(END, f'-' * 91 + '\n')
    texto_recibo.insert(
        END, f' Sub_Total: \t\t\t{var_subtotal.get()}\n')
    texto_recibo.insert(
...
def guardar():
    ...
    # cooredenadas iniciales
    x, y = 50, 800
    for linea in info_recibo.split("\n"):
        ...
        if y < 50:
            ...
            y = 800
    c.save()
    messagebox.showinfo('Informacion', 'Su recibo ha sido guardado en PDF')
...
aplicacion.geometry('1225x630+0+0')
...
etiqueta_titulo = Label(
    panel_superior, text='Sistema de Facturacion', fg='azure4', font=('Dosis', 58), bg='burlywood', width=20)
...
lista_comidas = ['pollo', 'cordero', 'salmon',
                 'panchos', 'pizza', 'papas fritas']

lista_bebidas = ['agua', 'soda', 'vino', 'coca-cola', 'sprite', 'fanta']

lista_postres = ['helado', 'fruta', 'brownies', 'flan', 'mousse', 'pastel']
...
for comida in lista_comidas:
    ...
    comida = Checkbutton(panel_comidas, text=comida.title(), font=(
        'Dosis', 19, 'bold'), onvalue=1, offvalue=0, variable=variable_comida[contador], command=revisar_check)
    ...
    cuadros_comida[contador] = Entry(panel_comidas, font=(
        'Dosis', 18, 'bold'), bd=1, width=6, state=DISABLED, textvariable=texto_comida[contador])
...
for bebida in lista_bebidas:
    ...
    # crear checkbutton
    varible_bebida.append('')
    varible_bebida[contador] = IntVar()
    bebida = Checkbutton(panel_bebidas, text=bebida.title(), font=(
        'Dosis', 19, 'bold'), onvalue=1, offvalue=0, variable=varible_bebida[contador], command=revisar_check)
    ...
    cuadros_bebida[contador] = Entry(panel_bebidas, font=(
        'Dosis', 18, 'bold'), bd=1, width=6, state=DISABLED, textvariable=texto_bebida[contador])
...
for postres in lista_postres:
    ...
    postres = Checkbutton(panel_postres, text=postres.title(), font=(
        'Dosis', 19, 'bold'), onvalue=1, offvalue=0, variable=variable_postres[contador], command=revisar_check)
    ...
    cuadros_postres[contador] = Entry(panel_postres, font=(
        'Dosis', 18, 'bold'), bd=1, width=6, state=DISABLED, textvariable=texto_postres[contador])
...
texto_costo_comida = Entry(panel_costos, font=(
    'Dosis', 12, 'bold'), bd=1, width=10, state='readonly', textvariable=var_costo_comida)
...
texto_costo_bebida = Entry(panel_costos, font=(
    'Dosis', 12, 'bold'), bd=1, width=10, state='readonly', textvariable=var_costo_bebida)
texto_costo_bebida.grid(row=1, column=1, padx=41)

# etiquetas de postres
...
texto_costo_postres = Entry(panel_costos, font=(
    'Dosis', 12, 'bold'), bd=1, width=10, state='readonly', textvariable=var_costo_postres)
...
texto_subtotal = Entry(panel_costos, font=(
    'Dosis', 12, 'bold'), bd=1, width=10, state='readonly', textvariable=var_subtotal)
texto_subtotal.grid(row=0, column=3, padx=41)

# etiquetas de impuestos
etiqueta_impuestos = Label(panel_costos, text='Impuestos', font=(
    'Dosis', 12, 'bold'), bg='azure4', fg='white')
etiqueta_impuestos.grid(row=1, column=2)

texto_impuestos = Entry(panel_costos, font=(
    'Dosis', 12, 'bold'), bd=1, width=10, state='readonly', textvariable=var_impuesto)
...
texto_total = Entry(panel_costos, font=(
    'Dosis', 12, 'bold'), bd=1, width=10, state='readonly', textvariable=var_total)
